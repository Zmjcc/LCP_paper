...
from generate_channel import generate_channel_dataset
import hdf5storage
args = parge_config()
Nt = args.Nt
Nr = args.Nr
...
#channels = generate_channel_dataset(5000,Nt = 64,Nr = 4, K = 20, B = 5, Lp = 10, f_s = 0.32*1e9, tap_range = 100)
save_root = dataset_root + 'converting_loss_test_dataset.mat'
channels = hdf5storage.loadmat(save_root)['H_list'][:,:,:,:K,:B]
...
def minus_sum_rate_loss(y_true, y_pred,Nt,Nr,dk,K,B,p,sigma_2,alpha_list):
    '''
    ...
    alpha_list = tf.cast(alpha_list, tf.complex128)
    ...
    for k in range(K):
        for rb in range(B):
            ...
            rate_k = tf.math.log(tf.linalg.det(tf.eye(Nr, dtype=tf.complex128) + SINR_k)) / tf.cast(tf.math.log(2.0),
                                                                                                    dtype=tf.complex128)
            sum_rate = sum_rate + alpha_list[:,k]*rate_k
...
def WMMSE(channel,Nt,Nr,dk,K,B,p,sigma_2,alpha_list,UW_return):
    ...
    def update_WMMSE_W(H,U,V):
        ...
        return W
    def update_WMMSE_V(H,U,W,alpha_list):
        temp_B = tf.zeros([Nt, Nt], dtype=tf.complex128)
        W = W * tf.tile(tf.reshape(alpha_list,[-1,1,1,1,K]),[1,dk,dk,B,1])
        for user in range(K):
            for rb in range(B):
    ...
    for i in range(50):
        ...
        V = update_WMMSE_V(H,U,W,alpha_list)
    V0 = V
    trace_VV = 0
    for user in range(K):
        trace_VV = trace_VV + tf.linalg.trace(
...
alpha_list = np.random.rand(batch_size,K)
#alpha_list = np.ones((batch_size,K))
alpha_norm,_ = tf.linalg.normalize(alpha_list,axis=1,ord=1)
alpha_list = alpha_norm * K
for i in range(total_iter):
    ...
    EZF_performance.append(np.mean(minus_sum_rate_loss(channel_iter, EZF_output,Nt,Nr,dk,K,B,p,sigma_2,alpha_list))/B)
    WMMSE_output = WMMSE(channel_iter,Nt=Nt,Nr = Nr,dk=dk,K=K,B=B,p=p,sigma_2=sigma_2,alpha_list=alpha_list,UW_return=False)
    WMMSE_performance.append(np.mean(minus_sum_rate_loss(channel_iter, WMMSE_output,Nt,Nr,dk,K,B,p,sigma_2,alpha_list))/B)
    channel_iter_miso = MIMO2MISO(channel_iter,Nt=Nt,Nr=Nr,dk=dk,K=K,B=B)

    alpha_miso_list = tf.reshape(tf.tile(tf.reshape(alpha_list,[-1,1,K]),[1,dk,1]),[-1,K*dk])
    U_iter_miso, W_iter_miso , V_iter_miso = WMMSE(channel_iter_miso,Nt=Nt,Nr=1,dk=1,K=K*dk,B=B,p = p,sigma_2=sigma_2,alpha_list = alpha_miso_list,UW_return=True)
    ...
    DUU_MISO_performance.append(np.mean(minus_sum_rate_loss(channel_iter, V_dual_pa,Nt,Nr,dk,K,B,p,sigma_2,alpha_list))/B)
...
fh = logging.FileHandler(dataset_root + 'data/Convert_Loss_%d_%d_%d_%d_%d_%d.log' % (Nt, Nr, K, dk,B, SNR_dB))
fh.setLevel(logging.DEBUG)
logger.addHandler(fh)
logger.addHandler(ch)
logger.info('Nt:%d Nr:%d K:%d dk:%d B:%d SNR_dB:%d' % (Nt, Nr, K, dk,B, SNR_dB))
